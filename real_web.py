...
# ----------------- MAIN LOOP -----------------
cursor.execute("SELECT srno, company_name, phone_number, jd_url FROM web_data LIMIT 200")
...
for comp in companies:
    ...
    # Update both website + email
    cursor.execute("""
        UPDATE web_data
        SET website = %s, email = %s, scraped_at = %s
        WHERE srno = %s
    """, (website, email, scraped_at, srno))

    conn.commit()
    print(f"✅ Updated: {company_name} | Website: {website} | Email: {email}")
